...
def build_events() -> list[Event]:
    g_text = """
2011 대학입학/2017년8월 대학졸업/2017년 9월 런던 대학원 입학/ 2018년 10월 런던 대학원 학기끝
주식회사 와이오오아키텍스튜디오 | 2025.04.02 ~
와이오오 | 2024.07.22 ~ 2025.04.02
(주)라이터 | 2022.03.01 ~ 2022.12.10
주식회사 로컬스티치 | 2021.03.01 ~ 2021.10.30
이케아코리아유한회사 | 2019.04.01 ~ 2021.01.01
"""
    h_text = "제대로 기간을 정하고 만난 연애없음. 지금만나는 사람도 확정짓진 않았는데 2026년 2월 24일부터 사귄거로 보면됨"

    events: list[Event] = []
    i = 1
    for t in split_items(g_text):
        w = parse_window(t)
        if not w:
            continue
        ...
        elif ("취업" in t) or ("시작" in t) or ("사업자" in t):
            et = "employment_start"
        elif ("졸업" in t) or ("학기끝" in t):
            et = "employment_end"
        events.append(Event(f"G{i:02d}", et, t, w[0], w[1]))
        i += 1
    for t in split_items(h_text):
        w = parse_window(t)
        if not w:
            continue
        if "결혼" in t:
...
def main() -> None:
    events = build_events()
    has_marriage_event = any(e.event_type == "marriage" for e in events)
    place = Place("Yongin", 37.2411, 127.1776, "Asia/Seoul")
    score_rows = []
    audit_rows = []
    for m in range(15 * 60 + 45, 16 * 60 + 15 + 1):
        hh, mm = m // 60, m % 60
        birth = BirthData(date(1992, 8, 12), time(hh, mm, 0), place)
        ...
        for ev in events:
            ...
            pool = [
                ...
                and any(pp in x[0] or pp in x[2] for pp in event_planets(ev.event_type, True))
            ]
            best = None
            for x in pool:
        ...
        eligible = g_miss <= 2 and (marriage_ok if has_marriage_event else True)
        score_rows.append(
            {
                "candidate_time": f"{hh:02d}:{mm:02d}",
